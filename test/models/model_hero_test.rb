...
class HeroTest < Minitest::Test
	...
	def test_switch_equip
		player = Model::Player.register("bringup_hero","image")[:player]
		heroMain  = Model::Hero.registerMainHero("301001",player)[:hero]
		Model::Item.addItem4Test(player)
		#------------------装备"
		##位置0上武器 
		ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 1,0,1)
		assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		##位置1上武器 
		ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 1,1,8)
		assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		#位置2 上坐骑 id = 23
		ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 1,2,14)
		assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		##"位置1换武器 id=3 (测试防具位置放装备)"
		# ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 1,1,3)
		# assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		#------------------兵法"
		# #"位置0上兵法 id = 23	  自带兵法"
		# ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 2,0,23)
		# assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		#"位置1上兵法 id = 24"
		ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 2,1,23)
		assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 2,1,24)
		assert_equal(Const::ErrorCode::Ok,ret[:retcode])
		ret = Model::Hero.switchEquipment(player , heroMain[:heroId] , 2,2,20)
		assert_equal(Const::ErrorCode::Ok,ret[:retcode])
